...
export class CoeDashboardService {
  ...
  async getSummary(period: 'today' | 'cycle' | 'year' = 'cycle') {
    ...
    // These 37 queries used to fire as one giant Promise.all — a single
    // dashboard load could momentarily claim every connection in the pool
    // (see prisma.service.ts's max: 6, sized against the pooler's true
    // headroom) and starve unrelated concurrent requests into
    // EMAXCONNSESSION. Running them in small sequential batches (awaited
    // one after another, not all at once) caps peak concurrent connection
    // demand from this one endpoint instead of spending the whole pool on
    // it at once.
    const [
      examsTotal,
      examsScheduled,
