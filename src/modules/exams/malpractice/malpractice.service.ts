...
export class MalpracticeService {
  ...
  async update(id: number, dto: UpdateMalpracticeDto) {
    ...
    if (dto.committee_sitting_at !== undefined) data.committee_sitting_at = new Date(dto.committee_sitting_at);
  ...
  async lookupStudent(query: LookupStudentQueryDto) {
    const student = await this.prisma.students.findFirst({
      where: {
        OR: [{ register_no: query.register_no }, { student_id_no: query.register_no }],
      },
      select: {
        id: true,
        register_no: true,
