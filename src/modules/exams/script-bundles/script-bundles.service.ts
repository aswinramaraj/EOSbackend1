...
import {
  computeArchiveLocation,
  computeRetentionUntil,
} from '../script-archive/script-archive.service';
...
export class ScriptBundlesService {
  ...
  async findAll(query: ListBundlesQueryDto) {
    ...
    if (query.is_second_valuation !== undefined)
      where.is_second_valuation = query.is_second_valuation;

    const mappingWhere: Prisma.exam_subject_mappingWhereInput = {};
    if (query.exam_id) mappingWhere.exam_id = query.exam_id;
    if (query.department_id)
      mappingWhere.classes = { department_id: query.department_id };
    ...
    return bundles.map((b) => ({
      ...
      entered_count: b.script_bundle_marks.filter(
        (m) => m.total_marks != null || m.is_absent,
      ).length,
      status: b.status,
  ...
  async getStats(examId: number) {
    const bundles = await this.prisma.script_bundles.findMany({
      ...
      include: {
        script_bundle_marks: { select: { total_marks: true, is_absent: true } },
      },
    });

    const scriptsValued = bundles.reduce(
      (sum, b) =>
        sum +
        b.script_bundle_marks.filter(
          (m) => m.total_marks != null || m.is_absent,
        ).length,
      0,
    );
    const totalScripts = bundles.reduce((sum, b) => sum + b.scripts_count, 0);
    const valuatorIds = new Set(
      bundles.map((b) => b.valuator_faculty_id).filter(Boolean),
    );
    ...
    const enteredToday = await this.prisma.script_bundle_marks.count({
      where: {
        bundle_id: { in: bundles.map((b) => b.id) },
        entered_at: { gte: startOfToday },
      },
    });
    ...
    return {
      ...
      second_valuation_count: bundles.filter((b) => b.is_second_valuation)
        .length,
  ...
  async allocate(dto: AllocateBundleDto) {
    ...
    if (dto.dummy_range_end < dto.dummy_range_start) {
      throw new BadRequestException({
        message: 'dummy_range_end must be >= dummy_range_start.',
        errorCode: 'INVALID_RANGE',
      });
    }

    // "Own-department scripts are excluded automatically" — a valuator can't be
    // assigned a bundle from their own department (conflict-of-interest guard).
    if (dto.valuator_faculty_id) {
      const valuator = await this.prisma.faculty.findUnique({
        where: { id: dto.valuator_faculty_id },
      ...
      if (
        mapping.classes?.department_id != null &&
        valuator.department_id === mapping.classes.department_id
      ) {
        throw new ConflictException({
          message:
            'This valuator belongs to the same department as the course and cannot be assigned this bundle.',
          errorCode: 'SAME_DEPARTMENT_VALUATOR',
        });
      }
    }
      ...
      const bundle = await tx.script_bundles.create({
        data: {
          ...
          expected_return_at: dto.expected_return_at
            ? new Date(dto.expected_return_at)
            : undefined,
  ...
  async getMarkSheet(bundleId: number) {
    ...
    const marks = await this.prisma.script_bundle_marks.findMany({
    ...
    return {
      bundle: {
        ...
        subject: bundle.exam_subject_mapping.subjects,
        valuator: bundle.faculty,
      },
      rows,
    };
  }
  ...
  async enterMark(bundleId: number, dto: EnterScriptMarkDto) {
    ...
    if (
      dto.dummy_number < bundle.dummy_range_start ||
      dto.dummy_number > bundle.dummy_range_end
    ) {
      throw new BadRequestException({
        message: 'Dummy number is outside this bundle range.',
        errorCode: 'DUMMY_OUT_OF_RANGE',
      });
    }

    const total = dto.is_absent
      ? null
      : (dto.part_a_marks ?? 0) +
        (dto.part_b_marks ?? 0) +
        (dto.part_c_marks ?? 0);
    ...
    if (bundle.status === 'allotted') {
      await this.prisma.script_bundles.update({
        where: { id: bundleId },
        data: { status: 'under_valuation' },
      });
    }
  ...
  async submitBundle(bundleId: number) {
    ...
    const enteredCount = await this.prisma.script_bundle_marks.count({
      where: { bundle_id: bundleId },
    });
    const expected = bundle.dummy_range_end - bundle.dummy_range_start + 1;
    if (enteredCount < expected) {
      throw new BadRequestException({
        message: `${expected - enteredCount} scripts still blank.`,
        errorCode: 'INCOMPLETE_SHEET',
      });
    }
    ...
    await this.transferMarksToExamMarks(
      bundle.id,
      bundle.exam_subject_mapping_id,
      bundle.valuator_faculty_id,
    );

    // A submitted bundle goes straight into the archive with a real,
    // deterministically-assigned shelf and the real 6-month retention clock
    // — the Answer Script Archive page has no separate "receive" step.
    const alreadyArchived = await this.prisma.script_archive_bundles.findUnique(
      { where: { bundle_id: bundleId } },
    );
    if (!alreadyArchived) {
      const existingArchivedCount =
        await this.prisma.script_archive_bundles.count();
      await this.prisma.script_archive_bundles.create({
        data: {
          bundle_id: bundleId,
          location_label: computeArchiveLocation(existingArchivedCount),
          retention_until: computeRetentionUntil(new Date()),
        },
      });
    }
   ...
   */
  private async transferMarksToExamMarks(
    bundleId: number,
    examSubjectMappingId: number,
    valuatorFacultyId: number | null,
  ) {
    ...
    const studentByDummy = new Map(
      scripts.map((s) => [s.dummy_number, s.student_id]),
    );
    ...
    for (const mark of marks) {
      ...
      const scaled =
        mark.is_absent || mark.total_marks == null
          ? null
          : Math.round(((Number(mark.total_marks) * externalMax) / 100) * 100) /
            100;
