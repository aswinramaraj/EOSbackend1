...
export class ExamRegistrationsService {
  ...
  async findAll(query: ListExamRegistrationsQueryDto) {
    ...
    const arrearRegs = await this.prisma.exam_registrations.findMany({
      where: {
        ...
        exams: { exam_category: 'arrear' },
      },
      select: { student_id: true },
    });
    const arrearsByStudent = new Map<number, number>();
    for (const r of arrearRegs)
      arrearsByStudent.set(
        r.student_id,
        (arrearsByStudent.get(r.student_id) ?? 0) + 1,
      );

    return rows.map((r) => ({
      ...r,
      courses_count:
        r.students.class_id != null
          ? (coursesByExamClass.get(`${r.exam_id}:${r.students.class_id}`) ?? 0)
          : 0,
  ...
  async getStats(examId?: number) {
    const where: Prisma.exam_registrationsWhereInput = examId
      ? { exam_id: examId }
      : {};
    const [total, approved, pending, rejected, feeUnpaid, feePartial, exam] =
      await Promise.all([
        ...
        this.prisma.exam_registrations.count({
          where: { ...where, fee_status: 'unpaid' },
        }),
        this.prisma.exam_registrations.count({
          where: { ...where, fee_status: 'partial' },
        }),
    ...
    return {
      ...
      fee_outstanding_amount:
        exam?.fee_amount != null
          ? Number(exam.fee_amount) * feePendingCount
          : null,
  ...
  async create(dto: CreateExamRegistrationDto) {
    ...
    if (existing) {
      throw new ConflictException({
        message: 'This student is already registered for this exam.',
    ...
    return this.prisma.exam_registrations.create({
      data: {
        exam_id: dto.exam_id,
        student_id: dto.student_id,
        fee_status: dto.fee_status ?? 'unpaid',
        reason: dto.reason,
      },
      include: INCLUDE,
    });
  ...
  async review(
    ...
    const updated = await this.prisma.exam_registrations.update({
      where: { id },
      data:
        dto.status === 'pending'
   ...
   */
  private async trySetRejectionReason(id: number, rejectionReason: string) {
    try {
      await this.prisma.$executeRaw`
        UPDATE exam_registrations SET rejection_reason = ${rejectionReason} WHERE id = ${id}
      `;
    } catch (err) {
      if (!isUndefinedColumnError(err, 'rejection_reason')) throw err;
    }
  }
