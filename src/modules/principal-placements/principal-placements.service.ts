 ...
 * year column on that table. `placement_drives.drive_type` (internship vs
 * full_time) is proposed in internship_drive_type.query.md — until it's run,
 * `internshipDriveIdFilter()` degrades to no filtering (every drive already
 * counts as full_time, so this overview's numbers are unaffected either way);
 * once it's run, every query below excludes internship drives so this stays
 * a full-time-placement-only view (the dedicated Internships feature owns
 * internship stats separately).
 */
@Injectable()
export class PrincipalPlacementsService {
   ...
   * an empty (no-op) filter until then, and once real, excludes internship
   * drives from this full-time-placement-only overview.
   */
  private async internshipDriveIdFilter(): Promise<Prisma.Sql> {
    try {
      const rows = await this.prisma.$queryRaw<{ id: number }[]>`
        SELECT id FROM placement_drives WHERE drive_type = 'internship'
      `;
      return rows.length > 0
        ? Prisma.sql`AND pd.id NOT IN (${Prisma.join(rows.map((r) => r.id))})`
        : Prisma.empty;
    } catch {
      return Prisma.empty;
    }
  ...
  async getOverview(user: JwtPayload) {
    ...
    const studentDeptJoin =
      departmentId !== undefined
        ? Prisma.sql`JOIN students st ON st.id = sda.student_id JOIN classes cl ON cl.id = st.class_id`
        : Prisma.empty;
    const studentDeptFilter =
      departmentId !== undefined
        ? Prisma.sql`AND cl.department_id = ${departmentId}`
        : Prisma.empty;
    const internshipFilter = await this.internshipDriveIdFilter();
    try {
      ...
      // even for Secretary.
      const companiesRows = await this.prisma.$queryRaw<
        CompaniesRow[]
      >(Prisma.sql`
        SELECT COUNT(DISTINCT pd.company_id)::bigint AS companies
      ...
      const currentPct =
        currentApplicants > 0
          ? Math.round((currentPlaced / currentApplicants) * 1000) / 10
          : null;
      const previousPct =
        previousApplicants > 0
          ? Math.round((previousPlaced / previousApplicants) * 1000) / 10
          : null;

      const deptMap = new Map(
        deptRows.map((row) => [
          row.department_id,
          Number(row.applicants) > 0
            ? Math.round((Number(row.placed) / Number(row.applicants)) * 1000) /
              10
            : null,
        ]),
      );
      ...
      return {
        ...
        placement_pct_delta:
          currentPct !== null && previousPct !== null
            ...
            : null,
        highest_package:
          offers?.highest_package !== null &&
          offers?.highest_package !== undefined
            ? Number(offers.highest_package)
            : null,
        highest_package_role: jobRoleRows[0]?.job_role ?? null,
        average_package:
          offers?.average_package !== null &&
          offers?.average_package !== undefined
            ? Math.round(Number(offers.average_package) * 100) / 100
            : null,
      ...
      this.logger.error(
        'DB error computing principal placements overview',
        err,
      );
