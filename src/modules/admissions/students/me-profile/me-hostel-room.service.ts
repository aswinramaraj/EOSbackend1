...
export class MeHostelRoomService {
  ...
  async getMyHostelRoom(userId: number) {
    ...
    return {
      ...
      block_name: mapping.hostel_rooms.hostel_blocks?.name ?? null,
      floor_name: mapping.hostel_rooms.hostel_floors?.name ?? null,
        ...
        select: {
          hostel_rooms: {
            select: {
              ...
              hostel_blocks: { select: { name: true } },
              hostel_floors: { select: { name: true } },
            },
          },
        },
      });
