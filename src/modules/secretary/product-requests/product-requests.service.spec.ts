...
describe('ProductRequestsService', () => {
  ...
  it('persists the reject reason to secretary_product_requests.remarks', async () => {
    const prisma = (service as unknown as { prisma: any }).prisma;
    prisma.secretary_product_requests.findUnique.mockResolvedValue({
      id: 5,
      title: 'Lab equipment',
    ...
    await service.review(
      5,
      { decision: 'rejected', remarks: 'Out of budget for this cycle' },
      1,
    );
    ...
    expect(strings.join('?')).toContain(
      'UPDATE secretary_product_requests SET remarks',
    );
    expect(values).toContain('Out of budget for this cycle');
  });
});
